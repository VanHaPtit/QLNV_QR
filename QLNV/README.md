...
| Employee | Chấm công, xem lương và quản lý thông tin cá nhân |
...
Lưu ý quan trọng: Khi một bản ghi lương bị xóa hoặc tính lại, hệ thống bắt buộc hoàn trả số ngày phép đã sử dụng trước đó để đảm bảo tính nhất quán dữ liệu.

### 4.6 System Configuration Module

Toàn bộ tham số nghiệp vụ được lưu trong cơ sở dữ liệu và có thể cấu hình qua giao diện Admin mà không cần chỉnh sửa mã nguồn.
...
- Xuất báo cáo lương, chấm công và nghỉ phép dưới dạng Excel hoặc PDF
...
Cách tiếp cận này giúp tách biệt rõ ràng trách nhiệm giữa các tầng, dễ kiểm thử, bảo trì và mở rộng khi bổ sung chức năng mới.
...
- QR Code có cơ chế chống replay attack
...
**Clone dự án**
```bash
git clone https://github.com/VanHaPtit/QLNV_QR.git
```
